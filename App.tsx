                ...
                {isLoggedIn && onlineUsersCount > 0 ? (
                  broadcastReady ? (
                    <Lock
                      ...
                      title="E2EE Broadcast Ready"
                    />
                  ) : (
                    <Unlock
                      size={14}
                      className="ml-auto text-orange-500"
                      title={`Broadcast E2EE Not Ready (${availableKeysCount}/${onlineUsersCount} Keys)`}
                    />
                  )
                ) : (
          ...
          <CardHeader className="p-3 border-b bg-gray-50/80 rounded-t-lg">
            <div className="flex justify-between items-center">
              <div className="flex items-center gap-2">
                ...
                {selectedUser ? (
                  privateReady ? (
                    <Lock size={16} className="text-green-600" title="E2EE Private Chat" />
                  ) : (
                    <Unlock size={16} className="text-orange-500" title="Waiting for key..." />
                  )
                ) : onlineUsersCount > 0 ? (
                  broadcastReady ? (
                    <Lock size={16} className="text-green-600" title="E2EE Broadcast" />
                  ) : (
                    <Unlock
                      size={16}
              ...
              <form onSubmit={handleSendMessage} className="w-full flex items-center gap-2">
                <Input
                  ...
                  placeholder={
                    selectedUser
                      ? privateReady
                        ? `Send E2EE message to ${selectedUser}...`
                        : `Waiting for ${selectedUser}'s key...`
                      : onlineUsersCount > 0
                      ? broadcastReady
                        ? `Send E2EE broadcast...`
                        : `Send broadcast (${availableKeysCount}/${onlineUsersCount} keys)...`
